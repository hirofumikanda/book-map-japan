...
- z10-14のconfidenceしきい値0.99を書店の密度に合わせて緩めるかどうか → **要判断**。実データ(3,376件)で測定した結果は次のとおり。
  ...
  z10-13ではチェーン店のみが表示対象だが、しきい値0.99によりその**約半数(52.2%)が広域で非表示**になる。「チェーン店を優先表示する」という狙いに対して取りこぼしが大きいと判断する場合は、z10-14のしきい値を0.95程度へ緩める余地がある。ビューアの`step`式の定数変更のみで完結するが、`bookstore-map-viewer`の仕様(ズームレベルに応じたconfidenceフィルタ)に数値が明記されているため、変更する場合は仕様の更新を伴う。
